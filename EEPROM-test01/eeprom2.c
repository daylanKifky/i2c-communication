...
int main(){
	...
	if ( write(file, buf, 4) != 4){
		...
		printf("Escrito el valor 0x%X/0x%X/0x%X en 0x%X\n", buf[1], buf[2], buf[3], CHIP_ADDR );
	...
	if ( read(file, bufin, 3) !=3 ){
		perror("No se pudo leer del dispositivo");
		exit(1);
	}

	printf("Leido el valor 0x%X/0x%X/0x%X en 0x%X\n", bufin[0],bufin[1],bufin[2], CHIP_ADDR);
